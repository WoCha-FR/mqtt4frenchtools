...
class InfosVille {
  ...
  /**
   * Met à jour le fichier vacances.json depuis les fichiers ICS officiels
   */
  async updateVacancesFile () {
    const urls = {
    ...
    try {
      const newVacances = { A: [], B: [], C: [], CORSE: [] }
      for (const [zone, url] of Object.entries(urls)) {
        const data = await this.request(url, null, { responseType: 'text' })
        if (data && typeof data === 'string') {
          newVacances[zone] = this.parseICS(data)
          logger.info(`Zone ${zone} : ${newVacances[zone].length} événements récupérés.`)
        }
      }

      if (Object.values(newVacances).some(z => z.length > 0)) {
        fs.writeFileSync('./data/vacances.json', JSON.stringify(newVacances), 'utf8')
        logger.info('Fichier vacances.json mis à jour avec succès (toutes vacances incluses)')
      }
    } catch (e) {
      logger.warn(`Erreur MAJ vacances ICS: ${e.message}`)
    }
  }

  /**
   * Parse robuste d'un fichier ICS scolaire
   *
   * @param {string} icsData Contenu du fichier ICS
  ...
  parseICS (icsData) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('BEGIN:VEVENT')) {
        ...
        if (currentEvent.debut && currentEvent.fin && currentEvent.nom) {
        ...
        if (line.startsWith('DTSTART')) {
          ...
          if (m) currentEvent.debut = `${m[1].slice(0, 4)}-${m[1].slice(4, 6)}-${m[1].slice(6, 8)}T00:00:00.000Z`
        } else if (line.startsWith('DTEND')) {
          const m = line.match(/:(\d{8})/)
          if (m) currentEvent.fin = `${m[1].slice(0, 4)}-${m[1].slice(4, 6)}-${m[1].slice(6, 8)}T00:00:00.000Z`
        } else if (line.startsWith('SUMMARY')) {
          let name = line.substring(line.indexOf(':') + 1).trim()
          name = name.replace(/\\/g, '').split(' - Zone')[0].split(' - Corse')[0].split(' (Zone')[0].trim()
          currentEvent.nom = name
        }
      }
    }
    return _.chain(events)
      .uniqBy(e => e.debut + e.nom)
      .sortBy(['debut'])
      .value()
  }

  /**
   * Met à jour les fichiers des jours fériés
   */
  async updateFeriesFiles () {
    const urls = {
      './data/metropole.json': 'https://calendrier.api.gouv.fr/jours-feries/metropole.json',
      './data/alsace.json': 'https://calendrier.api.gouv.fr/jours-feries/alsace-moselle.json'
    }
    for (const [file, url] of Object.entries(urls)) {
      try {
        ...
        if (data && !_.isEmpty(data)) {
          ...
          logger.info(`Fichier ${file} mis à jour via API`)
        }
      } catch (e) {
        logger.warn(`Erreur MAJ feries ${file}: ${e.message}`)
      }
    }
  }
   ...
   */
  async request (path, params = null, options = {}) {
    // Axios Config
    const config = {
      ...this.requestConfig,
       ...options,
